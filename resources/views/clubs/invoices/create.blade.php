...
@section('form-content')
    ...
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body ">
                <form method="post" action="{{route('clubs.invoices.store',$club->id)}}">
                    @csrf
                    <div class="form-group  @error('client_name') has-danger @enderror">
                        <label for="client_name">{{__('names.f_name')}}</label>
                        <input type="text" class="form-control" name="client_name" value="{{old('client_name')}}" id="client_name"  placeholder="{{__('names.f_name')}}">
                        @error('client_name')
                    ...
                    <div class="form-group  @error('date') has-danger @enderror">
                        ...
                        <input type="date" class="form-control" name="date" value="{{old('date')}}" id="date"  placeholder="{{__('names.date')}}">
                        @error('date')
                        <div class="text-danger">{{$message}}</div>
                        @enderror
                    </div>
