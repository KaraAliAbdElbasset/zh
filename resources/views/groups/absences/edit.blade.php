    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body ">
                    <form method="post" action="{{route('absences.update',[$a->id])}}">
                        @csrf
                        @method('PUT')
                        <div class="form-group  @error('reason') has-danger @enderror">
                            <label for="reason">{{__('names.reason')}}</label>
                            <input type="text" class="form-control" name="reason" value="{{old('reason',$a->reason)}}" id="reason"  placeholder="{{__('names.reason')}}">
                            @error('reason')
